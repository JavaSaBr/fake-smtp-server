...
public class EmailFactory {
  ...
  public Email convert(RawData rawData) throws IOException {
    try {
      ...
      var mimeMessage = new MimeMessage(session, rawData.getContentAsStream());
  ...
  private Email createPlainOrHtmlMail(
      RawData rawData,
      String subject,
  ...
  private Email createMultipartMail(RawData rawData, String subject, Multipart multipart)
  ...
  private void appendMultipartBodyParts(Email email, RawData rawData, Multipart multipart)
      throws MessagingException, IOException {
  ...
  private void appendMultipartContent(Email email, RawData rawData, BodyPart part)
      throws MessagingException, IOException {
    ...
    } else if (partContentType == ContentType.MULTIPART_RELATED
        || partContentType == ContentType.MULTIPART_ALTERNATIVE) {
  ...
  private Optional<EmailContent> createEmailContent(
      RawData rawData,
      ContentType contentType,
      Object messageContent) {

    var data = Optional
        .ofNullable(Objects.toString(messageContent, null))
